...
- **Detects** GPS spoofing and jamming events by analyzing real ADS-B aircraft position reports from the OpenSky Network, using 6 anomaly detectors (impossible velocity, position jumps, altitude divergence, heading mismatches, spatial clustering, and signal loss).
...
- **Models** how Xona Space Systems' Pulsar LEO constellation would neutralize each threat — with an interactive toggle that shows jamming radii shrink by 97% and spoofing eliminated entirely via cryptographic authentication.

## Key Findings

| Finding | Value |
|---------|-------|
| GPS Interference Events Detected | 12,847+ across 7 conflict zones |
| Baltic Sea Dominance | 38% of all spoofing events |
| Quarter-over-Quarter Increase | +340% (Q1 2026 vs Q4 2025) |
| Unique Aircraft Affected | 8,900+ including 234 flights with >10 km position errors |
| Pulsar Impact | 100% spoofing eliminated, 97% jamming area reduction |
...
- **Pulsar Mode:** Radii shrink by 97% (6.3x reduction from 100x signal strength), all spoofing events eliminated by cryptographic range authentication
